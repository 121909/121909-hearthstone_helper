...
发布 cohort 为：插件 `0.4.9`、规则集 `0.3.3`、HDT `1.53.11`、炉石
构建 `247416`。
...
旧版 `0.4.4 / 0.3.1`、`0.4.5 / 0.3.2`、`0.4.6 / 0.3.3` 和
`0.4.7 / 0.3.3` 和 `0.4.8 / 0.3.3` 的诊断数据不能与当前
`0.4.9 / 0.3.3` 混用。升级前先归档旧插件数据，以下操作不会删除旧证据：
...
if (Test-Path "$hdtData\DiscardAdvisor") {
    Move-Item "$hdtData\DiscardAdvisor" "$hdtData\DiscardAdvisor-pre-0.4.9-$stamp"
}
```
...
| `UnsupportedPatch` 或指纹不匹配 | 更新到最新项目源码并重装插件。`0.4.9` 对当前 HDT `CardDefs.base.xml` 接受哈希 `1D9CF031FB1FE37A39FDF4F515702BCC2425EB71BA8BE0A236948372B15A38BB`；其他哈希请保留诊断日志以便排查。 |
