...
发布 cohort 为：插件 `0.4.13`、Windows 运行器 `0.1.5`、规则集
...
`Logs\Power.log`，先等待 `MULLIGAN_STATE=INPUT`，再等待默认 8 秒让卡牌和
确认按钮动画完成后才点击；较慢机器可传入 `-MulliganUiSettleSeconds 12`
...
| 起手卡牌已出现但确认按钮尚未出现便发生点击 | 确认运行器至少为 `0.1.5`；它在日志标记后默认再等待 8 秒。慢速设备可传入 `-MulliganUiSettleSeconds 12` 或更高值。 |
