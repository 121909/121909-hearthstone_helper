...
发布 cohort 为：插件 `0.4.8`、规则集 `0.3.3`、HDT `1.53.11`、炉石
构建 `247416`。
...
`0.4.7 / 0.3.3` 的诊断数据不能与当前 `0.4.8 / 0.3.3` 混用。升级前先归档旧插件数据，以下操作不会删除旧证据：
...
if (Test-Path "$hdtData\DiscardAdvisor") {
    Move-Item "$hdtData\DiscardAdvisor" "$hdtData\DiscardAdvisor-pre-0.4.8-$stamp"
}
```
...
| `UnsupportedPatch` 或指纹不匹配 | 更新到最新项目源码并重装插件。若 `0.4.8` 仍出现该状态，计算 `CardDefs.base.xml` 的 SHA-256 并保留诊断日志以便排查。 |
