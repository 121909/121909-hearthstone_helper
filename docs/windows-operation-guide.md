...
发布 cohort 为：插件 `0.4.7`、规则集 `0.3.3`、HDT `1.53.11`、炉石
构建 `247416`。
...
旧版 `0.4.4 / 0.3.1`、`0.4.5 / 0.3.2` 和 `0.4.6 / 0.3.3` 的诊断数据不能与当前
`0.4.7 / 0.3.3` 混用。升级前先归档旧插件数据，以下操作不会删除旧证据：
...
if (Test-Path "$hdtData\DiscardAdvisor") {
    Move-Item "$hdtData\DiscardAdvisor" "$hdtData\DiscardAdvisor-pre-0.4.7-$stamp"
}
```
